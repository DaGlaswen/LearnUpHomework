...
public class JavaBank implements Bank {
    ...
    public int[] calculateInterest(int years, int[] investments) {
        // Этот метод принимает ежегодные пополнения и возвращает массив годовых процентов
        // Данный метод предполагает что проценты возвращаются к клиенту каждый год, а не хранятся в этом банке
